...
public:
    ...
    int8_t G_biWidth;
    int8_t G_biHeight;
    BmpDecoder(String fileName);
    ~BmpDecoder();
};

BmpDecoder::BmpDecoder(String fileName)
{
    ...
    else
    {
        ...
        for (uint8_t i = _biHeight - 1; i >= 0; i--)
        {
            for (uint8_t j = 0; j < _biWidth; j++)
            {
                BitMap_buffer[i * _biWidth + j] = readWord();
                Serial.println(F("解析中..."));
            }
...
BmpDecoder::~BmpDecoder()
{
    //析构函数
    free(BitMap_buffer);
    Serial.println("释放了BmpDecoder对象");
}
#endif
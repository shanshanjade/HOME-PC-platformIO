...
{
    myBright = map(bright.toInt(),0,100,0,255);
...
void setup(){
  ...
  Blinker.begin(auth,ssid,pswd);  //Blinker开始
  ...
  //   if (millis()> 60000 && Blinker.status() == PRO_WLAN_CONNECTING) //如果系统运行时间大于1分钟并且是“设备连接网络中”状态，就重置配网信息，让系统进入AP配网状态
  //   {
  //     Blinker.reset();
  //   }
  //   if (millis() > 90000 && Blinker.status() == PRO_WLAN_SMARTCONFIG_BEGIN) //如果设备运行1.5分钟了，并且是“等待智能配网状态”，就变为AP配网模式；
  //   { 

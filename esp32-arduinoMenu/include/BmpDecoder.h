/*
BmpDecoder使用方法
1. 在全局中声明BMPDecoder myDecoder;
2. 在使用中
		myDecoder.setFile(imageFile)

		myDecoder.G_biWidth;
		myDecoder.G_biHeight;
		myDecoder.BitMap_buffer;
...
public:
	...
	void setFile(File imageFile)
	{
		...
		{
			_isRevOrder = true;
			// Serial.println(F("图像是正向序列，为非正常序列"));
		}
		else
		{
			_isRevOrder = false;
		...
		_skipByte = 4 - ((_biBitCount * _biWidth * 4 / 32) % 4);
		if (_skipByte == 4)
			_skipByte = 0;
		// Serial.print(F("图像每行的补齐位是:"));
		// Serial.println(_skipByte);
		//***************************************************************************
		BitMap_buffer = new uint16_t[ _bfSize/2 + 1]; //为适应特殊情况 不能使用_biSizeImage来分配内存， 要使用_bfSize
		// BitMap_buffer = (uint16_t *)malloc(_biSizeImage);
		//***************************************************************************
		// if (BitMap_buffer != NULL)
		//     Serial.println("BitMap_buffer 申请成功");
		if (BitMap_buffer == NULL)
			Serial.println("BitMap_buffer 没有申请成功");
...
inline int BmpDecoder::readByte()
{
	return _file.read();
}
inline int BmpDecoder::readWord()
{
	int b0 = readByte();
	int b1 = readByte();
	return (b1 << 8) | b0;
}
inline long BmpDecoder::readDword()
{
	long b0 = readByte();
	long b1 = readByte();
	long b2 = readByte();

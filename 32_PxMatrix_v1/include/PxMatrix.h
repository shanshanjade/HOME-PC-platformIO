...
// Defines the speed of the SPI bus (reducing this may help if you experience noisy images) 20000000
#ifndef PxMATRIX_SPI_FREQUENCY
#define PxMATRIX_SPI_FREQUENCY 20000000    //提高此值可以解决频闪问题，但高于40000000 会出现重影现象,zao
#endif
...
class PxMATRIX : public Adafruit_GFX {
 public:
  ...
  uint16_t colorHSV(uint8_t h, uint8_t s, uint8_t v);
 ...
 // Set the block pattern {ABCD, DBCA} (default is ABCD)
  ...
  inline void drawBitmapInvert(int16_t x, int16_t y, const uint8_t bitmap[],
...
uint16_t PxMATRIX::colorHSV(uint8_t h, uint8_t s, uint8_t v) {
  ...
  uint8_t r,g,b;
	...
	i = h / 42.5f;
	// int difs = (h%42.5f); // factorial part of h
  int difs = fmod((double)h,42.5);
...
void PxMATRIX::latch(uint16_t show_time )
{
  ...
  if (_driver_chip==SHIFT)
  {
    ...
    digitalWrite(_LATCH_PIN,HIGH);
    // delayMicroseconds(10);    // <<< remove this
    digitalWrite(_LATCH_PIN,LOW);
...
void PxMATRIX::drawBitmapInvert(int16_t x, int16_t y, const uint8_t bitmap[],
                              int16_t w, int16_t h, uint16_t color) {
